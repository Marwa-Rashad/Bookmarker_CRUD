...
function show() {
  ...
  for (var i = 0; i < sitesList.length; i++) {
    content +=
      `        <div class="d-flex row align-items-center px-0 py-2 shadow mb-4">
        <h4 class="mr-5 col-sm-6">` +
      sitesList[i].name +
      `</h4>
        <a href="https://` +
      sitesList[i].url +
      `" class="bg-info text-white p-2 rounded ml-5 mr-3" target="_blank">Visit</a>
        <button type="button" class=" btn btn-warning mr-2" onclick="updateDetails(` +
      i +
      `);">Update</button>
        <button type="button" class=" btn btn-danger mr-2" onclick="deleteSite(` +
      i +
      `);">
            Delete
        </button>
...
function updateDetails(i) {
  document.getElementById("btn").innerHTML =
    `<button class="btn btn-info my-3" onclick="saveUpdatedData(` +
    i +
    `)">Update</button>
`;
  var siteName = document.getElementById("siteName");
...
function saveUpdatedData(i) {
  ...
  document.getElementById(
    "btn"
  ).innerHTML = `<button class="btn btn-info my-3" onclick="submit()">Submit</button>`;
  clear();
  show();
}
